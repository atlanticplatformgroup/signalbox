...
See [sanitized evidence](RECOVERY-EVIDENCE.json) and the [walkthrough guide](DEMO-RECORDING.md). Provider responses, original session history, and credentials remain in private local recovery storage. The hosted instance now runs these changes. A subsequent hosted verification reused the checked artifact, denied production over MCP, authorized staging, and verified the worker-published file over HTTPS. This deployment used no additional inference or Sandbox calls. See [hosted evidence](HOSTED-RELEASE-EVIDENCE.json). The recorded walkthrough still describes the earlier local run.
